 ...
 * This is the {@link SQLException} throwing equivalent of {@link DoubleSupplier}.
 */
@FunctionalInterface
public interface DoubleSQLSupplier {
     ...
     */
    static DoubleSupplier unchecked(DoubleSQLSupplier supplier) {
     ...
     * @throws NullPointerException If {@code supplier} is {@code null}.
     */
    static DoubleSQLSupplier checked(DoubleSupplier supplier) {
        Objects.requireNonNull(supplier);
        return () -> {
            try {
                return supplier.getAsDouble();
